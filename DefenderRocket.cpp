...
//         t = t_new;
//     }
void DefenderRocket::calculateV(AtackRocket*, float) {
    std::vector<float> p1=p->getParametrs();
    p1[1]=0.0f; p1[2]=0.0f; p1[3]=0.0f; p1[4]=0.0f;
    p->setParametrs(p1);
}
...
void DefenderRocket::calculateTrajectory(AtackRocket* target, float dt) {
    if(current_fuel<=0.0f) return; // тут типа только гравитация если топлива нет
    ...
    p1[0]=std::atan2(p1[3],std::sqrt(p1[1]*p1[1]+p1[2]*p1[2]))*180.0f/M_PI;
    p->setParametrs(p1);
}
...
void DefenderRocket::update(float deltaTime, AtackRocket* target) {
    if (!is_active or is_destroyed) return;
    if (target->isActive() and getDistanceToTarget(target)<KILL_RADIUS) {
