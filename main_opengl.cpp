...
const float CELL_SZ=150.0f;
const int N_ATK=N_CELLS;
const float MIN_DIST=300.0f;
const float DETECT_DIST=1500.0f;
const int TRAIL_LEN=120;

struct Cam{
    float ax=40.f,ay=25.f;//Углы поворота камеры
    float dist=11000.f;//Дистанция до точки обзора
    float cx=WORLD_W/2,cy=WORLD_H/2,cz=1000.f;//Точка обзора
    double mx=0,my=0;//Позиция мыши
    bool down=false;//Зажата ли кнопка мыши
};
...
int winW=1280,winH=720;//Размеры окна
Cam cam;//Состояние камеры

std::vector<std::vector<Pt3>> atkTrail,defTrail;//Следы атакеров и защитников
std::vector<Boom> booms;//Активные взрывы
int nHit=0;//Количество уничтоженных атакеров
bool done=false;//Флаг завершения симуляции
...
void onBtn(GLFWwindow*,int btn,int act,int){
    if(btn==GLFW_MOUSE_BUTTON_LEFT)cam.down=(act==GLFW_PRESS);//Отслеживаем левую кнопку
}

void onMove(GLFWwindow*,double x,double y){
    if(cam.down){//Вращаем камеру при перетаскивании
        cam.ax+=(float)(x-cam.mx)*0.4f;
        cam.ay-=(float)(y-cam.my)*0.4f;
        cam.ay=std::max(-89.f,std::min(89.f,cam.ay));//Ограничиваем вертикальный угол
    }
    cam.mx=x;cam.my=y;
}

void onScroll(GLFWwindow*,double,double dy){
    cam.dist-=(float)dy*300.f;//Приближение/отдаление
    cam.dist=std::max(500.f,std::min(20000.f,cam.dist));//Ограничиваем диапазон зума
}

void onKey(GLFWwindow* w,int k,int,int act,int){
    if(k==GLFW_KEY_ESCAPE&&act==GLFW_PRESS)glfwSetWindowShouldClose(w,1);//Выход по Escape
}
...
void drawCell(float cx,float cy,float sz){
    float h=sz/2,z=2.f;//Половина размера и высота отрисовки
    glColor4f(0.1f,0.6f,0.2f,0.5f);//Полупрозрачная заливка
    glBegin(GL_QUADS);
    glVertex3f(cx-h,cy-h,z);glVertex3f(cx+h,cy-h,z);
    glVertex3f(cx+h,cy+h,z);glVertex3f(cx-h,cy+h,z);
    glEnd();
    glLineWidth(2.f);glColor3f(0.2f,1.f,0.3f);//Рамка
    glBegin(GL_LINE_LOOP);
    glVertex3f(cx-h,cy-h,z);glVertex3f(cx+h,cy-h,z);
    glVertex3f(cx+h,cy+h,z);glVertex3f(cx-h,cy+h,z);
    glEnd();
    glPointSize(6.f);glColor3f(0.5f,1.f,0.5f);//Центр ячейки
    glBegin(GL_POINTS);glVertex3f(cx,cy,z+1);glEnd();
}

void drawRocket(float x,float y,float z,Col3 c,float sc=1.f){
    glPushMatrix();glTranslatef(x,y,z);
    glPointSize(10.f*sc);glColor3f(c.r*1.5f,c.g*1.5f,c.b*1.5f);//Яркая точка
    glBegin(GL_POINTS);glVertex3f(0,0,0);glEnd();
    float r0=8.f*sc,h0=25.f*sc;//Радиус и высота корпуса
    glColor3f(c.r,c.g,c.b);//Основной цвет
    glBegin(GL_TRIANGLE_FAN);glVertex3f(0,0,h0);
    for(int i=0;i<=8;++i){float a=i*(2.f*(float)M_PI/8);glVertex3f(r0*cosf(a),r0*sinf(a),0);}
    glEnd();
    glColor3f(c.r*0.6f,c.g*0.6f,c.b*0.6f);//Темная нижняя часть
...
void drawTrail(const std::vector<Pt3>& t,Col3 c){
    if(t.size()<2)return;//Нужно минимум 2 точки для линии
    glLineWidth(2.f);
    glBegin(GL_LINE_STRIP);
    for(int i=0;i<(int)t.size();++i){
        glColor4f(c.r,c.g,c.b,(float)i/t.size()*0.7f);//Прозрачность затухает к хвосту
...
void drawBoom(const Boom& b){
    float t=1.f-b.life/b.maxLife,r=200.f*t,a=b.life/b.maxLife;//Радиус взрыва и прозрачность
    glPointSize(4.f);
    glBegin(GL_POINTS);//Облако частиц
    for(int i=0;i<40;++i){
    ...
    glEnd();
    glLineWidth(2.f);
    glBegin(GL_LINE_LOOP);//Круг на земле
    for(int i=0;i<32;++i){
...
void drawHUD(int hit,int total,bool simDone){
    ...
    glOrtho(0,winW,0,winH,-1,1);//Переключение в 2D режим для HUD
...
bool inCell(float x,float y,Cell* cell){
    auto c=cell->getCorners();//Получаем углы ячейки
...
Map* makeMap(RNG& rng,float z0){
    ...
    Corner* cm3=new Corner(WORLD_W,WORLD_H,z0),*cm4=new Corner(0,WORLD_H,z0);//Углы карты
    std::vector<Cell*> cells;
    std::vector<std::pair<float,float>> pos;//Позиции центров ячеек
    for(int tr=0;(int)cells.size()<N_CELLS&&tr<50000;++tr){
        ...
        bool bad=false;
        //Проверяем минимальное расстояние до других ячеек
...
AtackRocket* makeAtk(RNG& rng,Map* map,int nc,float tx,float ty){
    ...
    for(int t=0;t<10000&&inside;++t){
        ...
        //Проверяем,что ракета не появляется внутри ячейки
        for(int i=0;i<nc;++i)if(inCell(x,y,map->getCellNumber(i))){inside=true;break;}
    }
    return new AtackRocket(100,x,y,0.f,tx,ty,0.f,0.f,500,3,25000.f);
}
...
int main(){
    ...
    win=glfwCreateWindow(winW,winH,"Rocket Defense",nullptr,nullptr);
    ...
    int nCells=0;
    while(nCells<N_CELLS&&gmap->getCellNumber(nCells))++nCells;//Считаем реальное количество ячеек
    ...
    //Создание атакующих ракет
    std::vector<AtackRocket*> atk;
    for(int i=0;i<N_ATK;++i){
        int t=rng.i(0,nCells-1);//Случайная целевая ячейка
        atk.push_back(makeAtk(rng,gmap,nCells,cellC[t].x,cellC[t].y));
    }

    //Назначение каждому защитнику ближайшего атакера как цели
    ...
    for(auto* a:atk)a->launch();//Запуск всех атакеров
    std::vector<bool> defReady(nCells,false);//Флаг-запущен ли защитник

    atkTrail.resize(N_ATK);
    defTrail.resize(nCells);

    const float DT=0.016f,MAX_T=300.f;//Шаг времени и макс.время симуляции
    float simT=0.f;
    Col3 cAtk={1.f,0.25f,0.1f},cDef={0.1f,0.4f,1.f},cExp={1.f,0.7f,0.1f};//Цвета
    ...
    while(!glfwWindowShouldClose(win)){
        ...
        if(!done&&simT<MAX_T){
            bool alive=false;//Есть ли живые ракеты
            ...
            //Запуск защитников при приближении атакера
            for(int i=0;i<nCells;++i){
            ...
            for(int i=0;i<nCells;++i){
                ...
                if(!wasDead&&defTgt[i]->isDestroyed()){//Атакер только что уничтожен
                    booms.push_back({defTgt[i]->getX(),defTgt[i]->getY(),defTgt[i]->getZ(),1.f,1.f,cExp});
                    ++nHit;
                }
            }
